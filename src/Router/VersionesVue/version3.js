    ...
    return (
      ...
      <React.Fragment>
        <div className="container col-12">
          <br/><br/>
          <p><strong>npm install -g @vue/cli:</strong> Es el CLI de vue, instalar globalmente el creador de proyectos de Vue.js. Para crear un proyecto nuevo <strong>vue create nombreapp</strong> y elegir la opcion <strong>Vue 3</strong>. Para crear un servidor virtual <strong>npm run serve</strong>.</p>
          <br/><br/>
          ...
          <p><strong>reactive():</strong> cuando queremos reagrupara valores reactivos en un solo estado. Para retornarlo hay que importar a toRefs() y en return( ...toRefs(nombre_del_metodo_reactive()) );</p>
          <ul>
          ...
          <p><strong>ref():</strong> coge un valor y devuelve un objeto reactivo, se usa cuando estamos trabajando fuera de data() pero seguimos queriendo un valor reactivo, ref() va a guardar el value del codigo y si hay cambiaos los va a mutar, despues de definir un ref() tenemos que definirlo en el return. const active = ref(0)  ---> nos va a dar un valor cero que va a ir cambiando. Si queremos devolver varios valores lo englosaremos dentro de un objeto, un array...</p>
          <ul>
          ...
          <p><strong>@keyup.enter="apply($event)":</strong> se pasa como parametro por ejemplo de un input y referencia que quieres hacer al pulsar la tecla enter.</p>

          <p><strong>emits[""]:</strong> para definir props de un hijo a un padre. para definir el evento de la funcion del padre se puede usar this.$emit().</p>
...
<p>beforeCreate: Apenas se está iniciando nuestro componente. correspone al setup() todo lo que contenga el setup() ya se esta almacenando aqui.
created: Ya está dispoinible toda la información reactiva del componente. correspone al setup() todo lo que contenga el setup() ya se esta almacenando aqui, por lo que si inicializamos un <strong>fetch()</strong> en setup() es como si lo hicieramos en created() o beforecreated().
