    ...
    return (
      ...
      <React.Fragment>
        <div className="container">
          ...
          <br/>
          <span className="subrayado"> La forma de pasar un componente dentro de otro es en forma de elemento, si le damos parámetros a estos elementos podemos pasarlos a esos componentes a través de <strong>this.props</strong>. También podemos heredar los props con <strong>this.props.children</strong>. </span> 
          <br/><br/>
          ...
          <br/>
          <span className="subrayado"> Si queremos simular un error usando async podemos hacerlo enviando throw new error('500: server error'), con el metodo <strong>throw</strong> colocándolo en el lugar de la petición.  </span> 
          <br/>
          <span className="subrayado"> El método error tiene por defecto un atributo llamado <strong>error.message</strong> para devolvernos el mensaje del error.</span>
          <br/>
          <br/>
          ...
          <br/>
          <p><strong>- Api externa "primera imagen":</strong> para trabajar con un api externo, lo haremos a través de un <span className="subrayado"> fetch()</span>, después usaremos un <span className="subrayado"> .json()</span> y con un método asíncrono. Para trabajar con apis debemos desplegar en el state: el mismo diagrama de valores que estemos usando en la api para mantener un state controlado, para finalizar el proyecto y desplegar su visualización en el return() debemos usar .map si la api se despliega en esa misma estancia en arrays, si usamos .map su primer valor de descomposición deberá ser el key= que nos sirve como método de estructuración en la api.</p>
        
          <p><strong>- Api interna con import (json) "segunda imagen":</strong> para trabajar con un api interno importaremos el api a nuestro componente del proyecto; <span className="subrayado"> (import proyecto from '../proyecto.json).</span> De esta forma tendremos guardado el json con la palabra clave proyecto. En este caso servirá con crear una función dándole la condición de al recibir el proyecto que se guarde en memoria dentro del state. Desglosaremos el state siguiendo la jerarquía del json con el que estamos trabajando, para mantener un state controlado.'<br/><span className="subrayado"> La diferencia con la api interna es que hay que importarla, no se utiliza un método asyncrono sino que una function en el componentdidmount() que desglose los datos; <strong>this.setState(data:proyecto)</strong>. y después se desglosa de igual forma porque esta guardado en menoría state:.</span></p>
          <br/>
          ...
          <span className="subrayado"> Vamos a utilizar fetch() en el momento de llamar a las apis, por defecto tiene un método html get pero podemos implementarle mas opciones agregando un request al fetch().</span>
          ...
          <br/>
          <span className="subrayado"> Los hooks en react son recomendables utilizarlos cuando queramos trabajar con los estados y tengamos que trabajar sobre <strong>funciónes</strong> en vez de clases. Para importarlos se usa<strong> import React, useState from 'react';</strong></span>
          <br/><br/>
